...
//   - 2026-07-30 改定 : 接待を伴う業態144店の公開により公開305店 × 25% ≒ 76件
// 割合(25%)は変えていない。母数(公開店舗数)が増えた分だけ上限を引き上げたもの。
const INSTAGRAM_LOGOS_MAX = 76;
{
...
// 18歳未満の入店が制限される。利用者が来店前に知っておくべき情報なので、対象カテゴリの
...
const AGE_RESTRICTION_NOTICE =
  "風営法上の接待飲食等営業に該当する店舗では、18歳未満の方の入店が制限されます。" +
  "また、酒類を提供する店舗では20歳未満の方は飲酒できません。" +
  "入店可否・年齢確認の取り扱いは各店舗の定めによりますので、詳細は各店舗にご確認ください。";

// 対象カテゴリのときだけ年齢制限の注記を返す(それ以外は空文字)。
function ageRestrictionNoticeHtml(categoryId) {
  if (!NIGHTLIFE_CATEGORIES.has(categoryId)) return "";
...
<article class="venue-detail">
  ...
  <section class="info-section">
    ...
    ${factsGridHtml}
    ${ageRestrictionNoticeHtml(v.category)}
  </section>
